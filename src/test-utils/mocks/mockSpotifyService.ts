// Shared mock factory for SpotifyService used in unit tests that should avoid
// importing the axios-based implementation. This provides deterministic
// implementations for common methods used across tests.

import { ApiError, ERROR_TYPES } from '../../services/apiErrorHandler';

export function makeMockSpotifyService() {
  // Return a simple ES5-style constructor class that tests can `new`.
  return class MockSpotifyService {
    accessToken: string;
    ...
    async searchTracks(query: string, options: any = {}) {
      ...
      while (retryCount <= maxRetries) {
        ...
        if (res.status === 429 && retryCount < maxRetries) {
          ...
          const waitTime = retryAfter ? Number(retryAfter) * 1000 : 1000; // Default to 1 second if no Retry-After header
    ...
    async removeTracksFromPlaylist(playlistId: string, request: any) {
      ...
      while (true) {
        ...
        // If rate limited, retry honoring Retry-After header (if present)
    ...
    async getPlaylistTracks(playlistId: string, options: any = {}) {
      ...
      const hasMore = false; // All pages have been fetched
      return { tracks: all, total, hasMore };
    }

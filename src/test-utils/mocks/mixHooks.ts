...
export const makeUseMixPreviewModule = (impl?: UseMixPreviewImpl) => {
  ...
  return {
    ...
    useMixPreview: () => {
      ...
      const generatePreview = vi.fn(async (...args: any[]) => {
        let result;
        if (impl) {
      ...
      const updateTrackOrder = vi.fn((newTracks: any[]) => {
        setState((s: any) => ({
          ...(s || {}),
...
export const makeUseMixGenerationModule = (impl?: UseMixGenImpl) => {
  ...
  return {
    ...
    useMixGeneration: () => ({
      ...
      generateMix: _mixFn,
      createPlaylist: vi.fn(async (name?: string, tracks?: unknown) => {
...
export const makeUseMixPreviewMock = (impl?: UseMixPreviewImpl) => {
  ...
  const mockModule = () => ({
    ...
    useMixPreview: () => ({
      ...
      clearPreview: vi.fn(),
      getPreviewTracks: vi.fn(() => [] as SpotifyTrack[]),
      _previewFn,
    }),
  });
...
export const makeUseMixGenerationMock = (impl?: UseMixGenImpl) => {
  ...
  const mockModule = () => ({
    ...
    useMixGeneration: () => ({
      ...
      generateMix: _mixFn,
      createPlaylist: vi.fn(async () => ({ id: 'created', name: 'created' })),
      reset: vi.fn(),
      _mixFn,
    }),
  });

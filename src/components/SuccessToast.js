      ...
      {mixedPlaylists.map((playlist, index) => (
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
            <div style={{ flex: 1 }}>
              <h3 style={{ margin: '0 0 8px 0', color: '#1DB954', fontSize: '16px' }}>
                🎉 Mixed Playlist Created!
              </h3>
              <p style={{ margin: '0 0 12px 0', fontSize: '14px', lineHeight: '1.4' }}>
                Your new playlist "{playlist.name}" has been created with {playlist.tracks?.total || playlist.tracks?.length || 0} songs
                {playlist.duration && (

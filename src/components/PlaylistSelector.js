...
const PlaylistSelector = memo(
  ({
    ...
    const extractPlaylistId = useCallback(url => {
    ...
    const isValidSpotifyLink = useCallback(input => {
    ...
    const isValidPlaylistUrl = useCallback(input => {
        ...
        try {
          ...
          while (true) {
            ...
            const tracks = tracksResponse.data.items
              .filter(
                item => item.track && item.track.id && item.track.duration_ms
    ...
    return (
      <div
        ...
        style={{
          position: 'relative',
          zIndex: showSearchResults ? 9999 : 'auto',
        }}
      >
          ...
          <div className="input-group">
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <input
                ...
                onFocus={() =>
                  playlistInput.trim() && setShowSearchResults(true)
                }
                onBlur={() =>
                    ...
                    <div
                      ...
                      onKeyDown={e => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault();
                          handleAddPlaylistByUrl(playlist.id);
                        }
                      }}
                      role="button"
                      tabIndex={0}

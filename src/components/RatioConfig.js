...
const ExampleMixDisplay = memo(
  ({ selectedPlaylists, ratioConfig, globalBalanceMethod }) => {
    const exampleMixData = useMemo(() => {
    ...
    return (
      ...
      >
        ...
        >
          <strong>🎯 {exampleMixData.exampleTitle}</strong>
...
const RatioConfig = memo(
  ({ selectedPlaylists, ratioConfig, onRatioUpdate, onPlaylistRemove }) => {
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    display: 'grid',
                    gridTemplateColumns: playlist.coverImage
                      ? 'auto auto 1fr'
                      : 'auto 1fr',
                    alignItems: 'center',
                    gap: '16px',
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ flex: 1, position: 'relative' }}>
                        <div
                          className="dual-range-background"
                          style={{
                            width: '100%',
                            height: '6px',
                    ...
                    >
                      🎲 Priority:{' '}
                      {(() => {
                        const totalWeight = Object.values(ratioConfig).reduce(
                          (sum, cfg) => sum + (cfg.weight || 20),
                          0
                        );
                        ...
                        if (weight <= 20)
                          return `Low (${weight}) - ~${percentage}% of mix`;
                        if (weight <= 40)
                          return `Normal (${weight}) - ~${percentage}% of mix`;
                        if (weight <= 60)
                          return `High (${weight}) - ~${percentage}% of mix`;
                        if (weight <= 80)
                          return `Top (${weight}) - ~${percentage}% of mix`;
                ...
                <button
                  onClick={() =>
                    onPlaylistRemove && onPlaylistRemove(playlist.id)
                  }
                  style={{
                    position: 'absolute',
                    top: '10px',

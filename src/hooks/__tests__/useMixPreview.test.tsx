...
// vi.mock factories run during the import phase, so anything they close over
...
// Minimal fixtures cast to `any`; only the fields the hook reads are set.
...
// A no-frills empty options object; the mixer is mocked so values are unused.
const noOptions = {} as any;

describe('useMixPreview', () => {
  ...
  test('generates preview with per-playlist stats and total duration for array result', async () => {
    ...
    await act(async () => {
      await utils.generatePreview(
        [makePlaylist('p1'), makePlaylist('p2')],
        {},
        noOptions
      );
    });
  ...
  test('missing spotify service sets error and calls onError', async () => {
    ...
    await act(async () => {
      await utils.generatePreview([makePlaylist('p1')], {}, noOptions);
    });

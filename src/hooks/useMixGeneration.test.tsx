...
describe('useMixGeneration', () => {
  ...
  test('throws when spotify service not available (no access token)', async () => {
    ...
    console.log('HOOK KEYS', Object.keys(ref.current || {}));
  ...
  test('concurrent generateMix calls only allow latest to update state', async () => {
    // simulate two generateMix calls where the first is slow and the second is fast
    ...
    await act(async () => {
      ...
      // p2 was the latest and should have been applied
    ...
    await act(async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'Mixing stopped early because these playlists ran out of songs: P1'
        )
      );
      warnSpy.mockRestore();
    });

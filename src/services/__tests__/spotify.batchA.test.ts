...
describe('SpotifyService - Batch A (pagination & batching)', () => {
  test('getPlaylistTracks paginates and calls onProgress', async () => {
    ...
    const largeTracks = Array.from({ length: total }).map((_, i) => ({
      id: `track_big_${i}`,
      name: `Big Track ${i}`,
      artists: [{ id: `artist_${i}`, name: `Artist ${i}`, uri: `spotify:artist:${i}` }],
      album: { id: `album_${i}`, name: `Album ${i}`, uri: `spotify:album:${i}` },
    ...
    server.use(
      rest.get('https://api.spotify.com/v1/playlists/:playlistId/tracks', (req, res, ctx) => {
        ...
        return res(
          ctx.json({
            items: slice.map(t => ({ track: t, added_at: new Date().toISOString(), added_by: { id: 'u' } })),
            total: largeTracks.length,
            limit,
            offset,
            next: offset + limit < largeTracks.length ? `?limit=${limit}&offset=${offset + limit}` : null,
          })
        );
      })
    );
    ...
    interface Progress { loaded: number; total: number; percentage: number }
  ...
  test('addTracksToPlaylist validates empty URIs and batches large requests', async () => {
    ...
    // @ts-ignore
    await expect(service.addTracksToPlaylist('pl_1', { uris: [] })).rejects.toBeDefined();

    // Batching: create 250 URIs -> should produce 3 POST calls (100,100,50)
    const totalUris = 250;
    const uris = Array.from({ length: totalUris }).map((_, i) => `spotify:track:batch_${i}`);
    ...
    server.use(
      rest.post('https://api.spotify.com/v1/playlists/:playlistId/tracks', async (req, res, ctx) => {
        ...
        return res(ctx.status(201), ctx.json({ snapshot_id: `snap_${postCalls}` }));
      })
    );

...
describe('SpotifyService - Batch G (getPlaylistTracks edge cases)', () => {
  test('sets total when only returned on first request and reports progress', async () => {
    ...
    const itemsPage1 = [{ track: { id: 't1', name: 'T1' }, added_at: 'now', added_by: { id: 'u' } }];
    const itemsPage2 = [
      { track: { id: 't2', name: 'T2' }, added_at: 'now', added_by: { id: 'u' } },
      { track: { id: 't3', name: 'T3' }, added_at: 'now', added_by: { id: 'u' } },
    ];

    let call = 0;
    server.use(
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      require('msw').rest.get('https://api.spotify.com/v1/playlists/:playlistId/tracks', (req: any, res: any, ctx: any) => {
        const url = new URL(req.url.toString());
        const offset = parseInt(url.searchParams.get('offset') || '0', 10);
        call++;
        if (offset === 0) {
          return res(ctx.json({ items: itemsPage1, total: 3, limit: 100, offset: 0 }));
        }
        return res(ctx.json({ items: itemsPage2, total: 3, limit: 100, offset: 100 }));
      })
    );

    const progress: any[] = [];
    const service = new SpotifyService('normal_token');
    const res = await service.getPlaylistTracks('pl_edge', { onProgress: (p: { loaded: number; total: number; percentage: number }) => progress.push(p) } as any);
  ...
  test('handles total zero and returns hasMore false and zero percentage', async () => {
    ...
    const prog: any[] = [];
    const service = new SpotifyService('normal_token');
    const res = await service.getPlaylistTracks('empty_pl', { onProgress: (p: any) => prog.push(p) } as any);

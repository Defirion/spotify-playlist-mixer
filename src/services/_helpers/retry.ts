...
export async function retryWithBackoff<T>(fn: () => Promise<T>, opts?: RetryOptions & { getRetryAfter?: () => number | null }) {
  ...
  while (true) {
    try {
      ...
      const wait = ra !== null && ra !== undefined ? ra * 1000 : base * Math.pow(2, attempt - 1);
      // jitter
